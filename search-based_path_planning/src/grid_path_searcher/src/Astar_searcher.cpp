...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2)
{
    ...
    */
    // 1. Manhattan
#ifdef DIS_MANHATTAN
    double d_x = abs(node1->coord[0] - node2->coord[0]);
    double d_y = abs(node1->coord[1] - node2->coord[1]);
    double d_z = abs(node1->coord[2] - node2->coord[2]);
    
    double Dis_Manhattan = d_x + d_y + d_z;
    ...
    Dis_Euclidean = sqrt(pow(node1->coord[0]- node2->coord[0],2) 
    ...
    Eigen::Vector3d d_array = {d_x, d_y, d_z}; //decrease
    ...
    Dis_Diagonal = sqrt(3) * d_array(2)
		 + sqrt(2) * (d_array(1) - d_array(2)) 
		 + d_array(1) - d_array(0);
...
{   
    ...
    ROS_WARN("[A*]{sucess} from start point to goal heu is  %f ", startPtr->fScore);
    //STEP 1: finish the AstarPathFinder::getHeu , which is the heuristic function
    startPtr -> id = 1; 
    ...
    */
	...
	currentPtr = openSet.begin() -> second;	
        ...
        */         
	    ...
	    double edge_costs = edgeCostSets[i];
	    tentative_gScore = currentPtr->gScore + edge_costs;
		...
		neighborPtr->nodeMapIt = openSet.insert(make_pair(neighborPtr->fScore, neighborPtr));
            ...
            else if(tentative_gScore <= neighborPtr->gScore){ //this node is in open set and need to judge if it needs to update, the "0" should be deleted when you are coding
                /*
                *
                *
...
{   
    ...
    */
    ...
    GridNodePtr current = terminatePtr;
